...
In ordinary mode use only `logseq_search`, `logseq_todos`, and
`logseq_append_journal`; in journal mode use only `logseq_journal_context` and
...
and `logseq_project_git`. The selected page comes from the process environment,
